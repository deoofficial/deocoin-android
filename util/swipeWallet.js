...
program
  ...
  .option('-f, --fee [n]', 'Optional, fee in DEO (default 0.0001 DEO), only if amount is not provided', parseFloat)
  .parse(process.argv);
        ...
        async.eachSeries(signers, function(dummy, cb) {
          ...
          function(err) {
            ...
            if (p.builder.isFullySigned()) {
              ...
              firstWallet.broadcastToDeocoinNetwork(ntxid, function(txid) {
                console.log('\t #######  SENT  TXID:', txid);
                process.exit(1);
              });
            } else {
